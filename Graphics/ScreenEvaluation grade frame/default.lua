...
t[#t+1] = Def.ActorFrame{
	...
	OnCommand=function(self)
		...
		Def.BitmapText{
			Font="Common Normal",
			OnCommand=function(self)
				self:zoom(0.9):x( -42*side(player) )
	...
	Def.BitmapText{
		 Font="Common Normal", Text=optionslist, OnCommand=function(self)
			self:xy(45,-112):zoom(0.6):shadowlength(2):wrapwidthpixels(400)
		end
	},
...
for index, ValTC in ipairs(JudgmentInfo.Names) do
	t[#t+1] = Def.ActorFrame{
		...
		Def.BitmapText{ Font="Common Normal", Text=ValTC,
		OnCommand=function(self)
			self:y(16*index):zoom(0.8):horizalign(left):shadowlength(0)
		end;
		};
	};
end
...
local PColor = {
	["PlayerNumber_P1"] = color("#FFFFFF"),
	["PlayerNumber_P2"] = color("#FFFFFF"),
};
...
for index, ScWin in ipairs(JudgmentInfo.Types) do
	t[#t+1] = Def.ActorFrame{
		...
		OnCommand=function(self)
			self:y(16*index):zoom(0.8):halign(1):diffuse( PlayerColor(player) )
...
for index, RCType in ipairs(JudgmentInfo.RadarVal) do
	...
	t[#t+1] = Def.ActorFrame{
		...
		Def.BitmapText{ Font="Common Normal",
		OnCommand=function(self)
			self:xy( -40, 16*index ):zoom(0.8):halign(1)
		...
		Def.BitmapText{ Font="Common Normal",
		OnCommand=function(self)
			self:y( 16*index ):zoom(0.8):halign(1)
		...
		OnCommand=function(self)
			self:xy( -125, 16*index -1 ):zoom(0.8):halign(0)
		end;
		};

		Def.BitmapText{ Font="Common Normal", Text="/",
		OnCommand=function(self)
			self:xy( -40, 16*index -1 ):zoom(0.8):halign(0):diffuse( PlayerColor(player) )
		end;
		};
...
t[#t+1] = Def.ActorFrame{
	...
	OnCommand=function(self)
		self:xy( 3, 16*7-2 ):zoom(0.8):halign(0):maxwidth(140)
	end;
	};

	Def.BitmapText{ Font="Common Normal";
	OnCommand=function(self)
		self:xy( 128, 16*7-1 ):zoom(0.8):halign(1)
