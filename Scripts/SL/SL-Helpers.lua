...
DetermineTimingWindow = function(offset)
	for i=1,5 do
		if math.abs(offset) < PREFSMAN:GetPreference("TimingWindowSecondsW"..i) * PREFSMAN:GetPreference("TimingWindowScale") then
			return i
		end
	end
	return 5
...
GetHoldJudgments = function()
	...
	for i,filename in ipairs(files) do
		...
		if string.match(filename, " %dx%d") then
			local name = filename:gsub(" %dx%d", ""):gsub(" %(doubleres%)", ""):gsub(".png", "")
			table.insert(hold_graphics, name)
		end
	end
...
GetComboFonts = function()
	local path = THEME:GetCurrentThemeDirectory().."Fonts/_combofonts/"
	...
	for directory_name in ivalues(dirs) do
		...
		if has_png and has_ini then
			...
			if directory_name == "Minecraft" then
