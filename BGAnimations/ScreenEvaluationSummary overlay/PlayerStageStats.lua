...
af[#af+1] = LoadFont("Common normal")..{
	InitCommand=function(self) self:zoom(0.8):horizalign(align1):x(col1x):y(-24) end,
	DrawStageCommand=function(self)
		if playerStats and score then

			-- trim off the % symbol
			local score = string.sub(FormatPercentScore(score),1,-1)
...
-- difficulty meter
af[#af+1] = LoadFont("Common normal")..{
	InitCommand=function(self) self:zoom(0.6):horizalign(align1):x(col1x):y(-1) end,
	DrawStageCommand=function(self)
...
for i=1,#TNSTypes do
	...
	af[#af+1] = LoadFont("Common normal")..{
		InitCommand=function(self)
			self:zoom(0.58):horizalign(align2):x(col2x):y(i*13 - 50)
